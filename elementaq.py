...
with st.sidebar:
    ...
    # Убираем жесткий Deadband, чтобы поправка была видна всегда, если CCV найден
    drift_window = st.number_input("CCV Match Window (+/- %)", 5.0, 50.0, 20.0)

# --- 2. HELPER FUNCTIONS ---

def is_below_loq(avg_val, mql_val):
    """Строгая проверка на LOQ из вашего кода."""
...
def to_num(val):
    """Преобразование только чистых чисел для математики."""
    if pd.isna(val): return None
    try:
        # Убираем флаги !, !! если они есть
        s = str(val).replace('!', '').strip()
        return float(s)
    except: return None

def get_target(type_str):
    """Извлечение номинала из Type (напр. CCV_0.5 -> 0.5)."""
...
if uploaded_file and st.button("🚀 Execute Analysis"):
    ...
    # PHASE 1: DRIFT (Для каждой пробы свой CCV)
    for b in blocks:
        for el in elements:
            ...
            # Считаем дрейф только если проба выше LOQ
            if raw_val and not is_below_loq(b['avg'][el], mql_val):
                ...
                for ccv in all_ccvs:
                    ...
                    if target and measured and measured > 0:
                        # Проверка соответствия диапазона (напр. +/- 20%)
    ...
    for b in blocks:
        ...
        if str(b['Type']).startswith('S'):
            ...
            dil = get_target(b['Type']) or 1.0 # Если в Type S_10, разбавление 10
            for el in elements:
                mql_v = to_num(b['mql'][el]) or 0.0
                if is_below_loq(b['avg'][el], mql_v):
    ...
    st.session_state.t1 = pd.DataFrame(t1_r)
    st.session_state.t2 = pd.DataFrame(t2_r)
    st.session_state.t3 = pd.DataFrame(t3_r)

# --- 4. OUTPUT ---
if st.session_state.t1 is not None:
    tab1, tab2, tab3 = st.tabs(["📊 Thresholds", "✅ Final Results", "📝 Math Log"])
    with tab1: st.dataframe(st.session_state.t1)
    with tab2: st.dataframe(st.session_state.t2)
    with tab3: st.dataframe(st.session_state.t3)
