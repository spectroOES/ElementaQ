...
st.title("🧪 ElementaQ: Analytical Engine v8.0")
...
if file:
    ...
    if st.button("🚀 Run Analysis"):
        ...
        for i in range(0, len(df_raw), 4):
            ...
            # Find the specific "Concentration average" row within the 4-row block
        ...
        # STEP 2: Calculate Global Drift Factor (f)
    ...
    if 'data' in st.session_state:
        s1, s2, s3, f_map = st.session_state['data']
        ...
        st.download_button("📥 Download Excel Report", output.getvalue(), "Report.xlsx")

        tabs = st.tabs(["📊 Raw Data", "✅ Final Results", "🔍 Audit Trail"])
        with tabs[0]: st.dataframe(s1)
        with tabs[1]: st.dataframe(s2)
        with tabs[2]:
            st.warning(f"Factors: { {k: round(v,4) for k,v in f_map.items() if v != 1.0} }")
            st.dataframe(s3)
